...
两种一种是顺序保存（大端序），一种是倒序保存（小端序） 。

不同的计算机如果其CPU不同，就有可能导致传输数据时发生错误，比如计算机A为大端序，计算机B为小端序，那么计算机A传送0x01020304的整型数据（在内存中表示为 00000001 00000010 00000011 00000100），传到到计算机B的内存块为00000001 00000010 00000011 00000100，但是计算机B却解释为0x04030201的数据，这就发生的歧义，所以我们定义了网络序，要求计算机传输数据时都需要转化为网络序再发送。

网络序的规则就是大端序。注意我们只需在设置`sockaddr_in`结构体信息时转化成网络序即可，在其他的数据收发过程不用手动转化，其过程是自动的，例如read和write。
